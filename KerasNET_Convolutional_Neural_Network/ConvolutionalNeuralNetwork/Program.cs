    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            KerasIterator trainData = trainDataGenerator.FlowFromDirectory(trainFolder, target_size: new Tuple<int, int>(224, 224),
                class_mode: "binary", batch_size: 16, seed: 42);
            ...
            var input = new Input(new Shape(224, 224, 3));
            var x = new Conv2D(64, kernelSize, activation: "relu").Set(input);
            x = new MaxPooling2D(poolSize).Set(x);

            x = new Conv2D(32, kernelSize, activation: "relu").Set(x);
            x = new MaxPooling2D(poolSize).Set(x);

            x = new Conv2D(16, kernelSize, activation: "relu").Set(x);
            x = new MaxPooling2D(poolSize).Set(x);

            x = new Conv2D(8, kernelSize, activation: "relu").Set(x);
            x = new MaxPooling2D(poolSize).Set(x);

            x = new Dense(128).Set(x);
            x = new Dropout(0.20f).Set(x);
            x = new Dense(64).Set(x);
            x = new Flatten().Set(x);

            var output = new Dense(1, activation: "sigmoid").Set(x);
            ...
            Keras.Callbacks.History? history1 = model.FitGenerator(trainData, epochs: 70, validation_data: testData, validation_steps: 12, steps_per_epoch: trainSteps, verbose: 1);
